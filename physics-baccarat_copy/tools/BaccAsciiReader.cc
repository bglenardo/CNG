...
A reader which extracts ASCII light and charge yields from Bacc .bin files.
********************************************************************************
Change log
	...
	for(int i=0; i<iNumRecords; i++) {
		...
		cPrimaryParName = new char[iPrimaryParNum*(kMaxCharacter +1) +2];
                ...
                for (int j = 0; j< iPrimaryParNum; j++){
                        ...
                        if (DEBUGGING) {
                                cout<< "iPrimaryParNum, ParName, KEn, PosX, DirX= "<< j<<", "<< strName<< ", "<< fPrimaryParEnergy_keV[j]
                                        << ", "<< fPrimaryParPosX_mm[j]<< ", "<< fPrimaryParDirX[j]<<endl;
                        }
                }
		...
		cCreatorProcess[cParticleNamePos]='\0';

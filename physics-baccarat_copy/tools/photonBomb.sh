...
# === Instructions ===
# 
# From base Bacc dir run the shell script ./tools/photonBomb.sh
# 
# If you run "./tools/photonBomb.sh --print" then Bacc will not
...
bombXY ()
{
    echo "#"                                     > ./BaccMacros/photonBombMultiRun.mac
    echo "# Macro created with photonBomb.sh"   >> ./BaccMacros/photonBombMultiRun.mac
    echo "#"                                    >> ./BaccMacros/photonBombMultiRun.mac

    cat ./BaccMacros/photonBomb.mac | grep -v '^Bacc/beamOn' \
            | grep -v '^pos/centre' | grep -v '^exit'\
                                                >> ./BaccMacros/photonBombMultiRun.mac

    echo "/gps/pos/centre $1 $2 15.95 cm"       >> ./BaccMacros/photonBombMultiRun.mac
    echo "/Bacc/beamOn 1"                     >> ./BaccMacros/photonBombMultiRun.mac
    echo "exit"                                 >> ./BaccMacros/photonBombMultiRun.mac
}
...
for (( i=-npoints-1; i<=npoints+1; i++ ));do
    ...
    # runs of Bacc without having to unhappily break out of it. To do so,
	...
	if test -a ./tools/.photonBombLock; then
        ...
        if [[ ${check} -eq '1' ]]; then
            ...
            if [ $# -eq 1 ] ; then  
                ...
                ./BaccExecutable ./BaccMacros/photonBombMultiRun.mac

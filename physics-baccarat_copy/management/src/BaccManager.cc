...
BaccManager::BaccManager()
{
	...
	historyFileStream << "/tmp/BaccCommandHistory_" << randomSeed << ".txt";
...
void BaccManager::SetSource( G4String spec )
{
    ...
    sspec >> volName >> sourceTypeName >> activity >> activityUnit;
    ...
    // If SingleDecay find the specified A and Z of nucleus to decay
    G4int aa = 0;
    G4int zz = 0;
    if( sourceTypeName.find("SingleDecay") < G4String::npos ) {
        ...
        if ( zzStr.length() > 0 ) 
            ...
            G4cout << "error in assigning SingleDecay, check syntax" << G4endl;
    ...
    G4double activityToSet = activity ;
		for( G4int i=0; i<(G4int)BaccComponents.size(); i++ ) {
            ...
            BaccComponents[i]->AddSource( currentSource, activityToSet, 
                ...
                singleParticleName, singleParticleEnergy);//, wimpMass);
            volumeFound = true;
        }
    }
    if( !volumeFound ) {
